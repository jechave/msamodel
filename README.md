...
| Function | Evaluates the model at | Returns |
|----|----|----|
| `calculate_profiles()` | selection strengths given as arguments | the profile |
| `calculate_decomposition()` | selection strengths given as arguments | the nested models and the three contributions |
| `predict_profiles()` | a fit, with standard errors | the profile |
| `predict_decomposition()` | a fit, with standard errors | the nested models and the three contributions |

The `calculate_*` pair explores how each constraint shapes the profiles;
the `predict_*` pair evaluates the model where the data puts it. Fits
come from `fit_lrmsd_msa_site()` on the residue axis and
`fit_lrmsd_msa_mode()` on the mode axis.

All four return a `$site` and a `$mode` tibble, holding the profiles on
the two axes. Their `metric` argument chooses the quantity: `"lrmsd"`
keeps the profile’s overall level, `"nlrmsd"` centres it on its mean,
which is the form a fit is made on.
