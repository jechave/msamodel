...
Inputs are a PDB file, active-site residue numbers, and, to fit, an
observed profile. Measuring that profile, by superimposing homologs over
an alignment, is outside this package.
...
error bands. Fits come from `fit_lrmsd_msa_site()` on the residue axis
and `fit_lrmsd_msa_mode()` on the mode axis.
