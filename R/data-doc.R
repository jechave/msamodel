...
#' protein. The fitter \code{\link{fit_lrmsd_msa_site}} takes the two columns as a
#' \code{(pdb_site, lrmsd_obs)} vector pair, so this is example data rather than a
#' required schema -- your own table may name its columns anything.
#'
...
#' @seealso \code{\link{fit_lrmsd_msa_site}}, \code{\link{add_site_properties}}
#' @family datasets
"znb_profile"
...
#' (\code{fit_lrmsd_msa_mode}), keyed by the mode index. The fitter takes the two
#' columns as a \code{(mode, lrmsd_obs)} vector pair, so this is example data rather
#' than a required schema.
#'
#' \strong{This data is SYNTHETIC, not empirical.} No empirical mode profile exists
#' yet (computing observed per-mode divergence from homologous structures + an alignment is
...
#' \describe{
#'   \item{mode}{Integer mode index (the response-mode key; modes are not
#'     structure-anchored, so there is no \code{pdb_site}).}
#'   \item{lrmsd_obs}{Synthetic observed log structural divergence per mode.}
#' }
#' @details Generated by fitting the SITE model to the real \code{znb_profile} by
#'   maximum likelihood (\code{fit_lrmsd_msa_site}) for a realistic \code{(a1, a2)},
#'   evaluating the mode forward map at that point, and adding seeded Gaussian noise
#'   (seed 2025, sd 0.30). See \code{data-raw/prepare_znb_data.R}.
#' @source SYNTHETIC -- not empirical. See Details and
#'   \code{data-raw/prepare_znb_data.R}.
#' @seealso \code{\link{fit_lrmsd_msa_mode}}, \code{\link{calculate_profiles}}
#' @family datasets
"znb_profile_n"
