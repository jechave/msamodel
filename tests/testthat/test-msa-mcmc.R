test_that("run_mcmc_msa validates prior ranges", {
  pp <- preprocess_spm(znb_spm)
  expect_error(
    run_mcmc_msa(pp, znb_profile, n_iter = 5, burn_in = 1,
                 a1_prior_range = c(0)),
    "a1_prior_range"
  )
  expect_error(
...
test_that("fixing a1 and a2 yields constant parameter columns", {
  ...
  pp <- preprocess_spm(znb_spm)
  samples <- run_mcmc_msa(pp, znb_profile, n_iter = 30, burn_in = 10,
                          fix_a1 = 3, fix_a2 = 7)
  expect_true(all(samples$a1 == 3))
  expect_true(all(samples$a2 == 7))
})

test_that("run_mcmc_msa returns a tibble of the expected shape", {
  pp <- preprocess_spm(znb_spm)
  samples <- run_mcmc_msa(pp, znb_profile, n_iter = 50, burn_in = 10,
                          fix_a1 = 2, fix_a2 = 3)
