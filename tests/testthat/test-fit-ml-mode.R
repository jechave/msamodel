...
test_that("fit_lrmsd_msa_mode returns the documented list shape", {
  pp <- znb_spm
  ml <- fit_lrmsd_msa_mode(pp, znb_profile_n$n, znb_profile_n$lrmsd_n_obs)
...
test_that("fit_lrmsd_msa_mode validates box bounds and init (fail loud)", {
  pp <- znb_spm
  expect_error(fit_lrmsd_msa_mode(pp, znb_profile_n$n, znb_profile_n$lrmsd_n_obs, a1_range = c(5)),
               "a1_range must be a vector of length 2")
  expect_error(fit_lrmsd_msa_mode(pp, znb_profile_n$n, znb_profile_n$lrmsd_n_obs, a1_range = c(10, 0)),
               "a1_range must be a vector of length 2 with min < max")
  expect_error(fit_lrmsd_msa_mode(pp, znb_profile_n$n, znb_profile_n$lrmsd_n_obs, log2_a2_plus1_range = c(13, 0)),
               "log2_a2_plus1_range must be a vector of length 2 with min < max")
  expect_error(fit_lrmsd_msa_mode(pp, znb_profile_n$n, znb_profile_n$lrmsd_n_obs, init = c(1)),
               "init must be a length-2 numeric")
  expect_error(fit_lrmsd_msa_mode(pp, znb_profile_n$n, znb_profile_n$lrmsd_n_obs, init = c(99, 1)),
               "init must lie within the box")
  expect_error(fit_lrmsd_msa_mode(pp, znb_profile_n$n, znb_profile_n$lrmsd_n_obs, init = c(a1 = 1, b = 1)),
...
test_that("znb_profile_n is reproducible from its seeded recipe", {
  ...
  ppm <- znb_spm
  site_fit <- fit_lrmsd_msa_site(znb_spm, znb_profile$pdb_site, znb_profile$lrmsd_i_obs)
