...
test_that("dr2_msa returns one finite, positive dr2 per site", {
  pp <- znb_spm
  d <- dr2_msa(pp$dr2mat_site, pp$energy_data, a1 = 2, a2 = 5)
...
test_that("lrmsd_nested_models builds the four variants at the right (a1,a2)", {
  pp <- znb_spm
  a1 <- 2; a2 <- 5
  nested <- lrmsd_nested_models(pp$dr2mat_site, pp$energy_data, a1, a2)
  ...
  expect_equal(nested$mm,  log(sqrt(dr2_msa(pp$dr2mat_site, pp$energy_data, 0,  0 ))))
  expect_equal(nested$ms,  log(sqrt(dr2_msa(pp$dr2mat_site, pp$energy_data, a1, 0 ))))
  expect_equal(nested$ma,  log(sqrt(dr2_msa(pp$dr2mat_site, pp$energy_data, 0,  a2))))
  expect_equal(nested$msa, log(sqrt(dr2_msa(pp$dr2mat_site, pp$energy_data, a1, a2))))
})

test_that("nlrmsd_msa centres the uncentred profile over the full support", {
  pp <- znb_spm
  a1 <- 2; a2 <- 5
  nc <- nlrmsd_msa(pp$dr2mat_site, pp$energy_data, a1, a2)
  ...
  lr <- lrmsd_msa(pp$dr2mat_site, pp$energy_data, a1, a2)
...
test_that("nlrmsd_msa agrees with predict_profiles's point profile", {
  ...
  fwd  <- nlrmsd_msa(pp$dr2mat_site, pp$energy_data, ml$a1, ml$a2)
...
test_that("nlrmsd_msa_decomposition contributions sum to the centred profile", {
  pp <- znb_spm
  a1 <- 2; a2 <- 5
  d <- nlrmsd_msa_decomposition(pp$dr2mat_site, pp$energy_data, a1, a2)
  ...
  prof <- nlrmsd_msa(pp$dr2mat_site, pp$energy_data, a1, a2)
...
test_that("nlrmsd_nested_models centres each variant independently", {
  pp <- znb_spm
  a1 <- 2; a2 <- 5
  nc <- nlrmsd_nested_models(pp$dr2mat_site, pp$energy_data, a1, a2)
  ...
  un <- lrmsd_nested_models(pp$dr2mat_site, pp$energy_data, a1, a2)
  expect_equal(nc$mm,  un$mm  - mean(un$mm))
  expect_equal(nc$msa, un$msa - mean(un$msa))
})

test_that("loglik_lrmsd_msa matches a frozen reference value", {
