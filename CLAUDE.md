...
Rscript dev/preview-vignette.R dr2n-analysis   # -> vignettes/<name>.html (git-ignored)
```
...
→ **preview** with `dev/preview-vignette.R`. Knitting requires the working tree to
...
re-knit `.Rmd`. **Never render the committed `vignettes/<name>.Rmd` in place** —
`html_vignette` renders `--self-contained`, which base64-embeds the figures and
then DELETES the `<name>_files/` dir the shipped `.Rmd` references. Use
`dev/preview-vignette.R` (renders a temp-dir copy; leaves the repo figures
intact).
