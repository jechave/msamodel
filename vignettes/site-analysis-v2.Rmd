...
vignette: >
  %\VignetteIndexEntry{Site analysis of structural divergence (v2)}
...
for real. The shipped `site-analysis-v2.Rmd` is PRE-RENDERED from this file.
Regenerate it after editing this file or after any change to the package code the
vignette exercises:

  Rscript -e 'knitr::knit("vignettes/site-analysis-v2.Rmd.orig", output = "vignettes/site-analysis-v2.Rmd")'

then commit both this file, the rendered `site-analysis-v2.Rmd`, and its figures.
...
Its sibling, the [mode analysis vignette](mode-analysis-v2.html), runs the same arc
...
`dr2_ijm` it causes; the verbs below read those from it.
...
We ask for the per-residue reading with `$site`; the site tibble has the index `i`, the
PDB number `pdb_site`, and the profile `lrmsd_i_msa` (the `msa` marks the full model,
with both selection pressures active; section 3 introduces the variants with one or both
switched off). The `which` argument picks the quantity: `"lrmsd"` (used through sections
...
```r
a1 <- 2; a2 <- 500
calculate_profiles(spm, a1, a2, which = "lrmsd")$site |> head()
#> # A tibble: 6 × 3
#>       i pdb_site lrmsd_i_msa
#>   <int>    <int>       <dbl>
#> 1     1       20       -2.40
#> 2     2       21       -3.31
#> 3     3       22       -3.44
...
these guesses with values estimated from data. The profile is keyed by `i` and
`pdb_site`, so we attach the two structural descriptors from `site_props`:


```r
profile_demo <- calculate_profiles(spm, a1, a2, which = "lrmsd")$site |>
  left_join(select(site_props, i, dactive, lrmsf), by = "i")
```
...
![plot of chunk profile-residue](site-analysis-v2_files/figure-html/profile-residue-1.png)
...
![plot of chunk profile-lrmsf](site-analysis-v2_files/figure-html/profile-lrmsf-1.png)
...
![plot of chunk profile-dactive](site-analysis-v2_files/figure-html/profile-dactive-1.png)
...
profiles (`lrmsd_i_mm`, `lrmsd_i_ms`, `lrmsd_i_ma`, `lrmsd_i_msa`) and their split
(`phi_mut`, `phi_stab`, `phi_act`). `which = "lrmsd"` keeps everything on the absolute
scale:


```r
decomp <- calculate_decomposition(spm, a1, a2, which = "lrmsd")$site   # a1 = 2, a2 = 500 from section 2
head(decomp)
#> # A tibble: 6 × 9
#>       i pdb_site lrmsd_i_mm lrmsd_i_ms lrmsd_i_ma lrmsd_i_msa phi_mut phi_stab
#>   <int>    <int>      <dbl>      <dbl>      <dbl>       <dbl>   <dbl>    <dbl>
#> 1     1       20      -2.42      -2.43      -2.40       -2.40   -2.42 -0.00387
#> 2     2       21      -3.35      -3.48      -3.29       -3.31   -3.35 -0.132  
#> 3     3       22      -3.27      -3.54      -3.20       -3.44   -3.27 -0.272  
#> 4     4       23      -3.62      -3.66      -3.44       -3.45   -3.62 -0.0364 
#> 5     5       24      -3.67      -3.83      -3.55       -3.64   -3.67 -0.164  
#> 6     6       25      -3.60      -3.82      -3.41       -3.65   -3.60 -0.221  
#> # ℹ 1 more variable: phi_act <dbl>
```
...
![plot of chunk nested-profiles](site-analysis-v2_files/figure-html/nested-profiles-1.png)
...
Those three `phi_*` columns are already in the `decomp` tibble above — the four nested
...
![plot of chunk decomp-fixed](site-analysis-v2_files/figure-html/decomp-fixed-1.png)
...
sweep_a1 <- purrr::map_dfr(a1_values, function(a1)
  calculate_profiles(spm, a1, 0, which = "lrmsd")$site |>
...
sweep_a2 <- purrr::map_dfr(a2_values, function(a2)
  calculate_profiles(spm, 0, a2, which = "lrmsd")$site |>
    rename(lrmsd = lrmsd_i_msa) |>
    mutate(a2 = a2) |>
    left_join(select(site_props, i, dactive, lrmsf), by = "i"))
```
...
![plot of chunk sweep-a1](site-analysis-v2_files/figure-html/sweep-a1-1.png)
...
![plot of chunk sweep-a2](site-analysis-v2_files/figure-html/sweep-a2-1.png)
...
The [inference methods vignette](inference-methods-v2.html) walks through this fit end
...
take the MSA prediction at the ML estimate — the `lrmsd_i_msa` column of the fitted
decomposition — join it to the observed profile, centre both over the compared residues,
and annotate with R²:
...
fit_msa <- calculate_decomposition(spm, ml$a1, ml$a2, which = "lrmsd")$site |>
...
![plot of chunk fit-profile](site-analysis-v2_files/figure-html/fit-profile-1.png)
...
![plot of chunk fit-scatter](site-analysis-v2_files/figure-html/fit-scatter-1.png)
...
![plot of chunk fit-vs-lrmsf](site-analysis-v2_files/figure-html/fit-vs-lrmsf-1.png)
...
![plot of chunk fit-vs-dactive](site-analysis-v2_files/figure-html/fit-vs-dactive-1.png)
...
`(ml$a1, ml$a2)` instead of the chosen strengths, and read the `phi_*` columns:


```r
phi_fit <- calculate_decomposition(spm, ml$a1, ml$a2, which = "lrmsd")$site
```
...
![plot of chunk decomp-fit](site-analysis-v2_files/figure-html/decomp-fit-1.png)
...
#> [1] purrr_1.2.0         ggplot2_4.0.1       tidyr_1.3.1        
...
#> [29] glue_1.8.0         R6_2.6.1           farver_2.1.2       magrittr_2.0.4    
#> [33] splines_4.2.1      scales_1.4.0       bio3d_2.4-5        labeling_0.4.3    
