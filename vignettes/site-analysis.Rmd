...
# The fit here is ML (deterministic); the seed is harmless but kept for a stable
# committed render.
set.seed(1024)
```
...
We use the worked example `1d6o_A` (peptidyl-prolyl isomerase FKBP1A), whose files ship
with the package in `extdata` and are read below with `system.file()`.
...
length, so expect minutes on a larger protein. The observed profile we fit to is loaded
later, in section 5.
