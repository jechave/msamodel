...
**per normal mode**; the two are companion views of one model.
...
The shipped `znb_wt` and `znb_spm` come from the setup pipeline below: from a
structure and its active-site residues you build an elastic network model and run
a single-point-mutation scan. `setup_enm()` takes a bio3d structure object
directly, so you read the file yourself — with `bio3d::read.pdb()` for legacy
`.pdb` or `bio3d::read.cif()` for mmCIF — and the active site is just a vector of
PDB residue numbers.
...
# Active-site residues (PDB numbering). These mark the red lines in every plot
# below and drive the activity-selection term.
...
`add_site_properties()`, which reads the wild-type ENM (`znb_wt`) and returns, per
site:
...
and `lrmsd_i_obs` — here `znb_profile`), `fit_lrmsd_i_msa_ml()` estimates `a1, a2`
by maximum likelihood, returning a point estimate plus an asymptotic covariance
from the Hessian at the optimum. It is fast and deterministic:
...
`msamodel` also provides a deterministic-posterior fit (adaptive Gauss–Hermite
quadrature, `fit_lrmsd_i_msa_agq()`) and tools to compare the two.
...
Section 3 decomposed the profile at a *chosen* `(a1, a2)`. The same sequential
$\phi_{\text{mut}}/\phi_{\text{stab}}/\phi_{\text{act}}$ decomposition applies to
the **fitted** profile — evaluate the four nested-model variants at the ML estimate
`(ml$a1, ml$a2)`, then split:


```r
nested_fit <- calculate_lrmsd_i_nested_models(pp, ml$a1, ml$a2)
phi_fit <- dplyr::bind_cols(
  nested_fit,
  calculate_msa_decomposition(nested_fit$lrmsd_i_mm, nested_fit$lrmsd_i_ms,
                              nested_fit$lrmsd_i_ma, nested_fit$lrmsd_i_msa)
)
```
