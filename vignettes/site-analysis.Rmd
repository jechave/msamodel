...
survive selection — gives `dr2_i` per residue, and `lrmsd = log(sqrt(dr2_i))`. Two
...
`model`/`frustrated` pick the spring rules. `generate_spm_data()` runs the scan:
...
minute or two; for this example the result also ships as `znb_spm`, so `data("znb_spm")`
loads it if you prefer to skip the wait. The observed profile we fit to is loaded later,
in section 5.

`spm` records, for each mutant, its energy changes and the per-residue divergence
`dr2_ijm` it causes; the `calculate_*` functions below read those from it.
...
The site index `i` is msamodel's internal 1..N residue index; `spm$site_map` is the
key from `i` to `pdb_site`, the PDB residue number used on the chain-plot x-axis.


```r
site_ids   <- spm$site_map$i   # the internal site indices i
site_props <- add_site_properties(tibble::tibble(i = site_ids), wt, pdb_site_active) |>
  mutate(lrmsf = log(sqrt(msf))) |>               # log RMSF (flexibility)
  left_join(spm$site_map, by = "i") |>            # attach pdb_site (i -> PDB number)
  select(i, pdb_site, dactive, lrmsf)
```

## 2. Divergence profile at given (a1, a2)

For any choice of `(a1, a2)`, `calculate_lrmsd_i_msa()` returns the per-residue
divergence profile. It has two columns — the site index `i` and the profile
`lrmsd_i_msa` (the `msa` marks the full model, with both selection pressures active;
section 3 introduces the variants with one or both switched off):


```r
a1 <- 2; a2 <- 500
calculate_lrmsd_i_msa(spm, a1, a2) |> head()
#> # A tibble: 6 × 2
#>       i lrmsd_i_msa
#>   <int>       <dbl>
#> 1     1       -2.40
...
these guesses with values estimated from data. The profile is keyed by `i`, so we attach
the structural descriptors from `site_props` (also keyed by `i`) — which also brings in
`pdb_site` for the chain-plot x-axis:


```r
profile_demo <- calculate_lrmsd_i_msa(spm, a1, a2) |>
...
```r
ggplot(profile_demo, aes(pdb_site, lrmsd_i_msa)) +
...
```r
ggplot(profile_demo, aes(lrmsf, lrmsd_i_msa)) +
...
```r
ggplot(profile_demo, aes(dactive, lrmsd_i_msa)) +
  geom_point(alpha = 0.5, colour = "grey30") +
...
**nested model variants**, each just the model (`calculate_lrmsd_i_msa()`) evaluated
with one or both selection pressures switched off:
...
`calculate_lrmsd_i_nested_models()` returns all four lrmsd profiles at once. Unlike
the single-profile `calculate_lrmsd_i_msa()`, it also carries `pdb_site` alongside `i`,
so you can join it to PDB-keyed data (as section 5 does) without going through
`site_map`:


```r
nested <- calculate_lrmsd_i_nested_models(spm, a1, a2)   # a1 = 2, a2 = 500 from section 2
head(nested)
#> # A tibble: 6 × 6
#>       i pdb_site lrmsd_i_mm lrmsd_i_ms lrmsd_i_ma lrmsd_i_msa
#>   <int>    <int>      <dbl>      <dbl>      <dbl>       <dbl>
#> 1     1       20      -2.42      -2.43      -2.40       -2.40
#> 2     2       21      -3.35      -3.48      -3.29       -3.31
#> 3     3       22      -3.27      -3.54      -3.20       -3.44
#> 4     4       23      -3.62      -3.66      -3.44       -3.45
#> 5     5       24      -3.67      -3.83      -3.55       -3.64
#> 6     6       25      -3.60      -3.82      -3.41       -3.65
```
...
nested_long <- nested |>
  tidyr::pivot_longer(c(lrmsd_i_mm, lrmsd_i_ms, lrmsd_i_ma, lrmsd_i_msa),
                      names_to = "model", values_to = "lrmsd") |>
  mutate(model = factor(toupper(sub("lrmsd_i_", "", model)),
                        levels = c("MM", "MS", "MA", "MSA")))
ggplot(nested_long, aes(pdb_site, lrmsd, colour = model)) +
...
activity selection's effect on its own, as the mirror of MS. The decomposition below
uses MM, MS, and MSA.) Writing $\phi$ for the contributions,
...
Use `calculate_lrmsd_i_nested_models()` when you want to *see* the four profiles;
use `calculate_decomposition_i_msa()` when you want the $\phi$ split directly. The
latter packages the arithmetic above: give it `spm` and `(a1, a2)` and it returns one
row per site with the three $\phi$ contributions.


```r
phi <- calculate_decomposition_i_msa(spm, a1, a2)   # a1 = 2, a2 = 500 from section 2
```
...
# Paper component colours (from the shap_decomposition chunks):
# mutation = orange, stability = blue, activity = red.
...
ggplot(phi_long, aes(pdb_site, phi, colour = component)) +
  geom_vline(xintercept = pdb_site_active, colour = "firebrick",
             alpha = 0.3, linewidth = 0.3) +
...
sweep_a1 <- purrr::map_dfr(a1_values, function(a1)
  calculate_lrmsd_i_msa(spm, a1, 0) |>
    rename(lrmsd = lrmsd_i_msa) |>
    mutate(a1 = a1) |>
    left_join(site_props, by = "i"))
...
sweep_a2 <- purrr::map_dfr(a2_values, function(a2)
  calculate_lrmsd_i_msa(spm, 0, a2) |>
    rename(lrmsd = lrmsd_i_msa) |>
    mutate(a2 = a2) |>
    left_join(site_props, by = "i"))
```
...
across the family; `lrmsd_i_obs` is the log of that per-residue variation. `msamodel`
does not do this step — you bring the result as a table with columns `pdb_site` and
`lrmsd_i_obs`. The package ships one for the worked example, `znb_profile`:
...
```r
ml <- fit_lrmsd_i_msa_ml(spm, znb_profile)   # spm from section 1; znb_profile is the target
c(a1 = ml$a1, a2 = ml$a2)
#>         a1         a2 
...
build the comparison table from the MSA prediction at the ML estimate, join it to the
observed profile, centre both over the compared residues, and annotate with R²:
...
fit_msa <- calculate_lrmsd_i_nested_models(spm, ml$a1, ml$a2) |>
...
```r
ggplot(cmp, aes(nlrmsd_i_obs, nlrmsd_fit)) +
...
```r
phi_fit <- calculate_decomposition_i_msa(spm, ml$a1, ml$a2)
```
...
phi_fit_long <- phi_fit |>
  tidyr::pivot_longer(c(phi_mut, phi_stab, phi_act),
                      names_to = "component", values_to = "phi") |>
  mutate(component = factor(labs_comp[component], levels = labs_comp))
ggplot(phi_fit_long, aes(pdb_site, phi, colour = component)) +
  geom_vline(xintercept = pdb_site_active, colour = "firebrick",
             alpha = 0.3, linewidth = 0.3) +
  geom_hline(yintercept = 0, colour = "grey80") +
  geom_line() +
...
#>  [9] evaluate_0.24.0    lifecycle_1.0.4    tibble_3.3.0       gtable_0.3.6      
#> [13] nlme_3.1-164       lattice_0.22-5     mgcv_1.9-1         pkgconfig_2.0.3   
#> [17] rlang_1.1.6        Matrix_1.5-4.1     cli_3.6.5          parallel_4.2.1    
#> [21] xfun_0.41          withr_3.0.2        knitr_1.43         generics_0.1.4    
#> [25] vctrs_0.6.5        grid_4.2.1         tidyselect_1.2.1   glue_1.8.0        
#> [29] R6_2.6.1           farver_2.1.2       purrr_1.2.0        magrittr_2.0.4    
#> [33] scales_1.4.0       splines_4.2.1      bio3d_2.4-5        labeling_0.4.3    
#> [37] S7_0.2.1           penm_0.2.0.9000
```
