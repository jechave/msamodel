...
1..N index; `pdb_site` is the PDB residue number, used on the chain-plot x-axis.
`spm$site_map` is the key between them, and every per-residue tibble the package returns
carries both.


```r
site_ids   <- spm$site_map$site   # the internal site indices
site_props <- add_site_properties(tibble::tibble(site = site_ids), wt, pdb_site_active) |>
  mutate(lrmsf = log(sqrt(msf))) |>               # log RMSF (flexibility)
  left_join(spm$site_map, by = "site") |>            # attach pdb_site (i -> PDB number)
  select(site, pdb_site, dactive, lrmsf)
```
...
these guesses with values estimated from data. The profile is keyed by `site` and
...
profile_demo <- calculate_profiles(spm, a1, a2, which = "lrmsd")$site |>
  left_join(select(site_props, site, dactive, lrmsf), by = "site")
```
...
#> [17] Matrix_1.5-4.1     desc_1.4.3         labeling_0.4.3     devtools_2.4.5    
#> [21] splines_4.2.1      stringr_1.6.0      htmlwidgets_1.6.4  S7_0.2.1          
#> [25] shiny_1.8.1.1      compiler_4.2.1     httpuv_1.6.13      xfun_0.41         
#> [29] pkgconfig_2.0.3    pkgbuild_1.4.4     mgcv_1.9-1         htmltools_0.5.8.1 
#> [33] tidyselect_1.2.1   tibble_3.3.0       viridisLite_0.4.2  withr_3.0.2       
#> [37] later_1.3.2        grid_4.2.1         nlme_3.1-164       xtable_1.8-4      
#> [41] gtable_0.3.6       lifecycle_1.0.4    magrittr_2.0.4     scales_1.4.0      
#> [45] cli_3.6.5          stringi_1.8.3      cachem_1.0.8       farver_2.1.2      
#> [49] fs_1.6.4           promises_1.2.1     remotes_2.5.0      ellipsis_0.3.2    
#> [53] generics_0.1.4     vctrs_0.6.5        RColorBrewer_1.1-3 tools_4.2.1       
#> [57] glue_1.8.0         processx_3.8.3     pkgload_1.5.2      parallel_4.2.1    
#> [61] fastmap_1.1.1      sessioninfo_1.2.2  memoise_2.0.1      knitr_1.43        
#> [65] profvis_0.3.8      usethis_2.2.3
```
