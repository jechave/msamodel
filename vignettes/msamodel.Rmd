...
| **`msamodel` computes**      | the elastic network model (`set_enm`), the mutation scan (`generate_spm`), the fit of `(a1, a2)` to your data (`fit_*`), and the predicted divergence profiles (`predict_profiles`) |
...
Note that `set_enm()` requires all five arguments — it has no defaults. The scan takes
a minute or two — it is the one slow step, and everything after it is fast.
