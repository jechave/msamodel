...
| **`msamodel` computes**      | the elastic network model (`setup_enm`), the mutation scan (`generate_spm`), the fit of `(a1, a2)` to your data (`fit_*`), and the predicted divergence profiles (`predict_profiles`) |
...
The arguments to `setup_enm()` and `generate_spm()` are the model's defaults; you
