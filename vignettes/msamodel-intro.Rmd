...
A third option estimates the **posterior** like the MCMC, but *deterministically*:
`fit_lrmsd_i_msa_agq()` approximates it by adaptive Gauss–Hermite quadrature, placing a
handful of nodes at the locations the Laplace (Gaussian) approximation suggests. For this
protein the posterior is near-Gaussian in `(a1, log2(a2 + 1))`, so a 5×5 grid (25
likelihood evaluations) reproduces the posterior essentially exactly — no chain, no seed,
no burn-in. It reports posterior means, standard deviations, and credible intervals on the
natural `(a1, a2)` scale:
...
To compare the arms on equal footing, we build **all three** profiles the same way:
...
# AGQ profile at the posterior mean (a1, a2).
...
r2_agq  <- r2(cmp_ml$nlrmsd_i_obs, cmp_ml$nlrmsd_agq)
```
...
Table: MCMC, ML and AGQ estimates of the selection parameters (all profiles predicted at a single (a1, a2); uncertainty is the MCMC/AGQ posterior sd or the ML asymptotic standard error)
...
The AGQ posterior mean and standard deviation match the MCMC's — at 25 deterministic
likelihood evaluations instead of thousands of Markov-chain steps.
...
Finally, the deterministic AGQ posterior directly against the MCMC posterior. Points
on the dashed 1:1 line mean the methods agree site-for-site; the R² here measures
**method-vs-method** agreement, not fit to data — AGQ recovers the sampled posterior at
a tiny fraction of the cost:


```r
r2_methods <- r2(cmp_ml$nlrmsd_mcmc, cmp_ml$nlrmsd_agq)
r2_methods
#> [1] 0.9998621
```

![plot of chunk agq-vs-mcmc](msamodel-intro_files/figure-html/agq-vs-mcmc-1.png)

Because AGQ carries the whole `(a1, a2)` posterior, it also propagates that uncertainty
to the **predicted profile**: `predict_lrmsd_i_agq()` returns, per residue, the posterior
mean and a credible band (the model prediction integrated over the posterior). Here the
95% band is shown against the observed profile (all mean-centered):

![plot of chunk agq-band](msamodel-intro_files/figure-html/agq-band-1.png)
