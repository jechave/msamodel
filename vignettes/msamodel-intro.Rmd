...
`fit_msa_ml()` maximises the *same* likelihood
([`calculate_loglik_msa()`]) over the same coordinates (`a1` and
...
```r
ml <- fit_msa_ml(pp, znb_profile)   # pp from section 1; znb_profile is the target
```
...
r2_mcmc <- r2(cmp_ml$nlrmsd_obs, cmp_ml$nlrmsd_mcmc)
r2_ml   <- r2(cmp_ml$nlrmsd_obs, cmp_ml$nlrmsd_ml)
```
