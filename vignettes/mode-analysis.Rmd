...
This vignette predicts structural divergence **per normal mode**. Its sibling, the
[site analysis vignette](site-analysis.html), predicts the same divergence **per
residue** with `calculate_dr2_i_msa()`; the two are companion views of one model.
...
The final section fits the mode model to data. No *empirical* per-mode divergence
profile exists yet (deriving `dr2_n` from structural alignments is out of scope for
`msamodel`), so that section fits a **synthetic** stand-in profile to exercise the
mode fit end-to-end — it is flagged as synthetic where it appears.
