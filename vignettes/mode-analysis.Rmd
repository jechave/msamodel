...
arc **per residue**; the two are companion views of one model.
...
The mode axis reuses the **same** single-point-mutation scan as the site axis —
only the reshaping differs, and there is no structure-reading step here. That scan,
`znb_spm`, is produced by the pipeline shown in the
[site analysis vignette's setup](site-analysis.html) (read structure → `setup_enm()`
→ `generate_spm_data(...)`); see `?generate_spm_data` for the generating parameters.
We reload it here. `preprocess_spm_mode()` is the mode counterpart of
`preprocess_spm()`: it reshapes the per-mutant scan into the `[mutant x mode]`
matrix the mode evaluator reweights.


```r
data("znb_spm")           # single-point-mutation scan (same as the site axis)
data("znb_profile_n")     # observed (SYNTHETIC) mode profile, the fit target
data("znb_profile")       # real site profile; the §5 truth the synthetic
                          # mode profile was built from (see ?znb_profile_n)
```
...
Mirroring the site fit, `fit_lrmsd_n_msa_ml()` estimates `a1, a2` by maximum
likelihood — maximising the mode-form profiled Gaussian log-likelihood
(`calculate_loglik_lrmsd_n_msa()`) over the same `(a1, log2(a2 + 1))`
coordinates — returning a point estimate plus an asymptotic covariance from the
Hessian. The target is `znb_profile_n` (columns `n`, `lrmsd_n_obs`), keyed by the
mode index `n`.
...
`(ml_n$a1, ml_n$a2)`, then split. (The fit is to synthetic data, so this decomposes
a synthetic-fitted profile.)


```r
nested_fit <- calculate_lrmsd_n_nested_models(pp_mode, ml_n$a1, ml_n$a2)
phi_fit <- dplyr::bind_cols(
  nested_fit,
  calculate_msa_decomposition(nested_fit$lrmsd_n_mm, nested_fit$lrmsd_n_ms,
                              nested_fit$lrmsd_n_ma, nested_fit$lrmsd_n_msa)
)
```
