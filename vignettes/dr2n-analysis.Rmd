...
**per site**: `calculate_dr2i_msa()` returns `dr2_i`, the selection-weighted mean
...
```r
data("znb_spm")
pp_mode <- preprocess_spm_mode(znb_spm)   # list: energy_data, dr2nmat
dim(pp_mode$dr2nmat)                       # mutants x modes
#> [1] 2280  678
```

## The mode-divergence profile at given (a1, a2)

For any `(a1, a2)`, `calculate_dr2n_msa()` returns the selection-weighted mean
...
```r
a1 <- 2; a2 <- 500
prof_mode <- calculate_dr2n_msa(pp_mode, a1, a2)
head(prof_mode)
#> # A tibble: 6 × 2
#>       n   dr2_n
#>   <int>   <dbl>
...
c(site_total = sum(znb_spm$dr2[[r]]),
  mode_total = sum(znb_spm$dr2n[[r]]))
...
#> [1] ggplot2_4.0.1  dplyr_1.1.4    msamodel_0.2.0
#> 
