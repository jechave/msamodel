...
vignette: >
  %\VignetteIndexEntry{msamodel: predicting structural divergence from a single-mutation scan (v2)}
...
for real. The shipped `msamodel-v2.Rmd` is PRE-RENDERED from this file. Regenerate it
...
exercises (run from INSIDE vignettes/ so fig.path resolves correctly):
  ...
  cd vignettes && Rscript -e 'knitr::knit("msamodel-v2.Rmd.orig", output = "msamodel-v2.Rmd")'

then commit both this file, the rendered `msamodel-v2.Rmd`, and its figures.
...
analysis](site-analysis-v2.html) vignette explains what each controls). The scan takes a
...
each residue, how much its position varies across the family; `lrmsd_i_obs` is the log
of that per-residue variation. `msamodel` does not do this step — you bring the result
as a table with columns `pdb_site` and `lrmsd_i_obs`. The package ships one for the
worked example, `znb_profile`:
...
```r
pred <- predict_profiles(ml, spm, which = "nlrmsd")
head(pred$site)
#> # A tibble: 6 × 4
...
The `which` argument picks the quantity: `"nlrmsd"` (used here) is the profile with its
...
Each tibble carries the profile value (`nlrmsd_i_msa` / `nlrmsd_n_msa`) and a standard
error (`..._se`) that propagates the fit's `(a1, a2)` uncertainty to every point. To draw
...
site <- pred$site |>
  mutate(lower = nlrmsd_i_msa - z * nlrmsd_i_msa_se,
         upper = nlrmsd_i_msa + z * nlrmsd_i_msa_se) |>
...
ggplot(site, aes(pdb_site)) +
  ...
  geom_line(aes(y = nlrmsd_i_msa, colour = "fitted (MSA)")) +
...
![plot of chunk panel-site](msamodel-v2_files/figure-html/panel-site-1.png)
...
be fit to its own data; see the [mode analysis](mode-analysis-v2.html) vignette.)


```r
ggplot(mode, aes(n)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = "grey70", alpha = 0.4) +
  geom_line(aes(y = nlrmsd_n_msa), colour = "grey30") +
  labs(title = "Per mode: predicted divergence (from the site fit)",
       x = "Normal-mode index n", y = "nlrmsd (shape)")
```

![plot of chunk panel-mode](msamodel-v2_files/figure-html/panel-mode-1.png)

The same evolutionary divergence, read per residue and per mode.

## Where to next

- **[Site analysis](site-analysis-v2.html)** — the per-residue arc in full: predict,
...
- **[Inference methods](inference-methods-v2.html)** — the maximum-likelihood fit end
